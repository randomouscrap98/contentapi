...
namespace contentapi.test
{
    ...
    public class ReadTestBase : UnitTestBase
    {
        ...
        public async Task<ReadTestUnit> CreateUnitAsync()
        {
            ...
            unit.commonContent = await contentService.WriteAsync(new ContentView() { name = "commonContent", parentId = 0, type="teft", permissions = new Dictionary<long, string>() {{0 , "cr" }} }, requester);
            unit.specialContent = await contentService.WriteAsync(new ContentView() { name = "specialContent", parentId = 0, type="teft", permissions = new Dictionary<long, string>() {{unit.specialUser.id , "cr" }} }, requester);
