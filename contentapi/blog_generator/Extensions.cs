        ...
        try
        {
            ...
            do
            {
                ...
                if(result.MessageType == WebSocketMessageType.Text) //If statement optimization, don't go checking the other paths
                    ...
                    throw new InvalidOperationException($"Client sent unsupported message type: binary");
            ...
            try {
                return JsonConvert.DeserializeObject<T>(readString) ?? throw new InvalidOperationException($"Couldn't parse an object of type {typeof(T)}");
            }
            catch(Exception ex) {
                throw new InvalidOperationException($"Readstring failure, string = '{readString}'", ex);
            }
        }
        finally
        {
