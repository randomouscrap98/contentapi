...
namespace contentapi.Services.Views
{
    ...
    public interface IViewReadService<V,S> where V : IIdView where S : IIdSearcher
    {
        Task<List<V>> SearchAsync(S search, Requester requester);
    }

    public interface IViewService<V,S> : IViewReadService<V,S> where V : IIdView where S : IIdSearcher
    {
    ...
    public interface IViewRevisionService<V,S> : IViewService<V,S> where V : IIdView where S : IIdSearcher
    {
